  ...
  return (
    <>
      ...
      <section className="flex flex-wrap justify-center pb-10">
        {newVehicles.every((vehicle) => vehicle?.car_type?.length === 0) && (
          <p className="w-full text-center">Car not found!</p>
        )}
        {(newVehicles.length !== 0 ? newVehicles : vehicles).map(
          (vehicleType: Vehicles) => {
            // if(vehicleType?.car_type?.length === 0) return <>Car not found!</>
            return (
              <div
                key={vehicleType?.id}
